...
int main(int argc, char* argv[])
{
    ...
    // parse command-line arguments; options followed by ':' expect an argumen, optarg is the current argument; in example below "hp:" are the supported options, if getopt encounters a foreign option it will return -1
    int opt;
    ...
    // ensure port is a non-negative short and path to server's root is specified; after loop above exits optind indexes the next argv pointer to process
    ...
    // start server, argv[optind] is path to root
    start(port, argv[optind]);

    // listen for SIGINT (aka control-c), if ctrl-c is input the program calls handler which stops execution
    ...
    while (true)
    {
        ...
        // close last client's socket,fff if any
        if (cfd != -1)
        {
            close(cfd);
            cfd = -1;
        ...
        if (connected())
        {
            // check for request, we are taking the address of a pointer &message so we can actually go to that address and actually put a pointer there; notice message is intialized with NULL
            if (request(&message, &length))
            {
                // extract message's request-line, this only extracts the first HTTP header, like GET / HTTP/1.1 and ignores the rest of the headers
                // http://www.w3.org/Protocols/rfc2616/rfc2616-sec5.html
                const char* haystack = message;
                // searches for the end of first line(end of the header) returns pointer to end of header and returns null if not found
                ...
                // copy the special characters \r\n as well, and add 1 space for the string terminator \0
                ...
                if (parse(line, abs_path, query))
                {
                    ...
                    // ensure path exists, all that it does is check if the file at path exists,returns 0 if file exists
...
const char* lookup(const char* path)
{
    // find the '.' in the path, if any; if found save address of '.' at pointer 'dot'
    char* dot = NULL;
    if ((dot = strchr(path, '.')) == NULL)
        return NULL;
...
/**
 *  s a request-line, storing its absoluter-path at abs_path 
 * and its query string at query, both of which are assumed
 * to be at least of length LimitRequestLine + 1.
 */
bool parse(const char* line, char* abs_path, char* query)
{
    // make sure the first 4 characters in the header contain the GET[space]
    if (strncmp("GET ", line, 4) != 0)
        {
        error(405);
        return false;
        }
    // check that the header ends with [space]HTTP/1.1\r\n and save pointer to [space] as 'space2' is not NULL
    char* space2 = NULL;   
    if ((space2 = strstr(line + 4," HTTP/1.1\r\n")) == NULL)
        {
        error(505);
        return false;
        }
        
    // make sure request target, which should start at the 5th character, starts with '/'
    if (*(line + 4) != '/')
        {
        error(501);
        return false;
    ...
    // make sure the request-target does not contain the a '' character
    if (strchr(line,'"') != NULL) 
        {
        error(400);
        return false;
    ...
    // QUERY AND ABSOLUTE PATH check for '?' which signals a query might follow; if '?' found save pointer to '?' in start_query, then increment saved pointer 1 to point to start of query substring
    // if after incrementing by 1 start_query points to a space that means there is no query substring
    // absolute path starts at line[5]; if QUERY found path ends before the '?'; if NO QUERY found then absolute path ends before 'space2'
    char* start_query = NULL;
    if ((start_query = strchr(line,'?')) != NULL)
        {
            // found the '?' character, let's check if it is followed by a query substring; if the character following '?' is ' ' then no substring present
            // if there is a substring present after '?' set the end of substring to be pointer space2 and copy substring into 'query'
            // if a query was found, the abolute path starts at 'line + 4' and ends at 'start_query - 1' which should now point to '?' 
            if (start_query++ != space2)
            {
                char* end_query = space2;
                strncpy(query,start_query,end_query - start_query);
                query[end_query-start_query + 1] = '\0';
                strncpy(abs_path, line+4, start_query - 1 - (line + 4));
            }    
            // we haven't found a query substring after the '?' 
            else 
                {
                    strcpy(query,"");
                    strncpy(abs_path, line+4, start_query - 1 - (line + 4));
        ...
        {
        strcpy(query,"");
        strncpy(abs_path, line + 4, space2 - (line + 4));
        }    
